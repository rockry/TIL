## 2017-11-05
- git rebase와 fetch
- fragment간 통신
- RxJava Hot Observable과 Cold Observable
- Dagger2

## 2017-11-02
- RxJava2 추가

## 2017-11-01
- RxJava2 : Disposable, Flowable, Subject ([link](./Android/EventBus&RxJava.md))
- Android : [FlavorDimension](https://academy.realm.io/kr/posts/gdg-seoul-flavordimension/)

## 2017-10-31
- EventBus, RxJava ([link](./Android/EventBus&RxJava.md))

## 2017-10-30
- Picasso, Glide

## 2017-10-24
- Interviews 정리

## 2017-10-19
- Interviews 정리 - GC

## 2017-10-18
- 안드로이드 개발자 이직 공부(이승현) #04/#05

## 2017-10-16
- Interviews 정리
### Android
- copyDependencies

## 2017-10-15
### etc
- 인터렉션 디자인의 6가지 원칙
- Interviews 정리
## 2017-10-12
- Interviews 정리
## 2017-10-11
- Interviews 정리
## 2017-10-10
- Interviews 정리
## 2017-10-09
- Design Pattern(Interviews) 정리

## 2017-09-29
### Java & Android
- HashMap vs HashTable update
- Floyd's Cycle-Finding Algorithm(tortoise and hare 알고리즘)
- 왜 java에서 static의 사용을 지양해야 하는가?http://tech.thegajago.com/2016/02/20/%EC%99%9C-%EC%9E%90%EB%B0%94%EC%97%90%EC%84%9C-static%EC%9D%98-%EC%82%AC%EC%9A%A9%EC%9D%84-%EC%A7%80%EC%96%91%ED%95%B4%EC%95%BC-%ED%95%98%EB%8A%94%EA%B0%80/ : 상태를 가질 필요가 없는 메소드들을 static으로 사용하는 것은 괜찮다고 생각함. 대신 그 클래스는 static하게 사용할 수 있는 메소드들만 모아 놓는게 좋다고 생각함.(ex. UtilityManager, Logger 등)
- Singleton 패턴

## 2017-09-28
## Java & Android
- 이승현님 Branch : Android 이직 공부 #01~#03
### etc
- Machine Learning 기초 세미나(with BetaGo)
    : y = Wx + B => Cost를 줄이는 W를 학습한다.(W = weight, B = Bais)
    : ?? + Softmax

## 2017-09-27
### Java & Android
- LinkedList
- JCF

## 2017-09-25
### Java & Android
- ArrayMap
- Retrofit, OkHttpClient(보류)
### Excercise
- 헬스 90min

## 2017-09-24
### Java & Android
- Java8 Optional
- 람다 내부 구조

## 2017-09-22
### Java & Android
- 람다
    - https://programmers.co.kr/learn/courses/9/lessons/915
    - http://d2.naver.com/helloworld/4911107
### etc
- github 유용한 단축키
- Closure

## 2017-09-21
### Java & Android
- MultiThread Android Sample
- Rx GuGuDan Example (유동 - RxAndroid https://brunch.co.kr/@yudong/56)

## 2017-09-20
### Java & Android
- 면접후기 정리 중

## 2017-09-19
### Java & Android
- BearCola Java 그리고 Android : 2017-06-21
    - View dispatchTouchEvent
    - Dalvik 과 ART 의 차이
    - Android N 변경사항
    - 안드로이드 런타임 퍼미션
    
## 2017-09-18
### Java & Android
- BearCola Java 그리고 Android : 2017-06-21
    - enum 의 장점
    - Volatile vs Synchronized
    - Java UnitTest

## 2017-09-17
### Java & Android
- BearCola Java 그리고 Android : 2017-06-19 / 2017-06-20
    - ConcurrentModificationException(X)
    - Equals 를 Override 할 때, 반드시 Override 해야하는 함수
    - Android 서비스는 메인 스레드에서 실행
    - HashMap 자료구조
    - Composite 패턴
    - 상속의 위험성
    - 객체지향언어의 4대 특징
    - 서비스 생명주기
    - 서비스에서 Bind 와 Start Service 를 섞어쓰지 않는 이유
### Algorithm
- KAKAO_Blind_모의문제 : 1,2

## 2017-09-16
### Java & Android
- Android Service
- SoftReference & WeakReference
- ListView & RecyclerView

## 2017-09-15
### Java & Android
- Android Thread
    - https://academy.realm.io/kr/posts/android-thread-looper-handler/
    - https://brunch.co.kr/@oemilk/45
    - https://www.youtube.com/watch?v=qk5F6Bxqhr4&index=1&list=PLWz5rJ2EKKc9CBxr3BVjPTPoDPLdPIFCE

## 2017-09-14
### Java & Android
- 유니코드
    - http://jsonobject.tistory.com/246

## 2017-09-13
### Java & Android
- 안드로이드 이미지 캐쉬
    - http://androi.tistory.com/130
    - https://kimdohyeong.gitbooks.io/android_official_training_kr/content/Caching_Bitmaps.html(Not Yet)
    - https://developer.android.com/topic/performance/graphics/index.html(Not Yet)
    - https://developer.android.com/topic/performance/graphics/cache-bitmap.html(Not Yet)
- android animation(더 깊이 볼 필요.)
    - https://material.io/guidelines/motion/material-motion.html#
    - https://labs.ribot.co.uk/exploring-meaningful-motion-on-android-1cd95a4bc61d
    - https://github.com/hitherejoe/animate
    - https://developer.android.com/training/material/animations.html
- Weak Reference, Soft Reference : http://itmining.tistory.com/10
- Android ListView 사용하기 : http://thdev.tech/androiddev/2016/10/30/Android-CustomListView-Sample.html
    - ListView / ViewHolder
    - 더 깊이 볼 내용 : RecyclerView / ArrayAdapter / LruCache / Retrofit / Chrome Custom Tabs

## 2017-09-13
### Java & Android
- BearCola Java 그리고 Android : 2017-06-18
    - Vector
    - 서비스에서 뷰를 띄울 수 있는가?
    - Intent Service 란?
    - Doze 모드란?
    - 허프만 코드 알고리즘
    - ListView vs RecyclerView 
    - Annotation 이란? 
    - Multi Dex 란?
    - NULL OBJECT 패턴(X)
    - Binary Search Tree

## 2017-09-12
### Java & Android
- BearCola Java 그리고 Android : 2017-06-04 / 2017-06-17
    - 안드로이드 뷰의 생성 과정
    - 안드로이드 액티비티 생명주기
    - 안드로이드에서 Enum 사용을 자제시켰는데 그 이유는 무엇인가?
    - 대용량 Bitmap 을 불러올 경우 메모리 문제가 있기 쉬운데 이럴 때 어떻게 처리하는가?
    - String Literal
    - Annotation 생성 방법
    - 프로세스와 스레드의 차이
    - ArrayList vs LinkedList
    - Quick Sort 
    - Iterator 를 쓰지 않고 직접 참조 시 문제점
    - DP란?
    - 제네릭이란?
    - Singleton 동기화 
    - Heap Data Stack 영역
    - Lambda 식이란?
- Udemy 코딩인터뷰 완벽준비 : 16
    - 16 : Java 지식문제 : 오버로딩 vs 오버라이딩 / HashMap 동작 원리 / final, finally, finalize() / ThreadLocal

## 2017-09-11
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 15
    - 15 : 수학 및 응용문제
- Programmers
    - 135 : x만큼 간격이 있는 n개의 숫자

## 2017-09-10
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 12, 13, 14
    - 12 : 비트연산
    - 13 : Sort
    - 14 : Search

## 2017-09-09
### Java & Android
- Stack과 Queue
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 9, 10
    - 9 : 큐
    - 10 : 트리/그래프

## 2017-09-08
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 8
    - 8 : 스택
### etc
- NamingRule
- 2의 보수

## 2017-09-07
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 7
    - 7 : 링크드리스트

## 2017-09-06
### Java & Android
- Fragment 기본 사용법
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 5-5, 6
    - 5-5 : Parentheses
    - 6 : Dynamic Programming (핵심 = memoization)
### Javascript
- Array 응용 방법

## 2017-09-05
### Java & Android
- RelativeLayout_Tip
- Network Processing In Android
- [Enum을 피해야되는 이유](https://brunch.co.kr/@oemilk/94)
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 5-1 ~ 5-4 복습
### Excercise
- 헬스 90min

## 2017-09-04:
### Java & Android
- JSON Parsing 
- BroadcastReceiver 동적 사용법
- ListView 기본 사용법
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 섹션5-재귀 (반복시청필요)
    - 5-1 : DiceCount
    - 5-2 : Painter
    - 5-3 : BitCombinations
    - 5-4 : Permutation

## 2017-09-03:
### Algorithm
- Udemy 코딩인터뷰 완벽준비 : 섹션1 ~ 섹션4 ([Src](https://github.com/rockry/algorithm/tree/master/udemy_%EC%BD%94%EB%94%A9%EC%9D%B8%ED%84%B0%EB%B7%B0%EC%99%84%EB%B2%BD%EC%A4%80%EB%B9%84))
    - 섹션1 : 강의소개
    - 섹션2 : 코딩면접 준비하기
    - 섹션3 : 기초이론
    - 섹션4 : 문자열
- Youtube 코딩인터뷰 완전분석 : 파이썬 1.1 ~ 1.4 ([Src](https://github.com/rockry/algorithm/tree/master/youtube_%EC%BD%94%EB%94%A9%EC%9D%B8%ED%84%B0%EB%B7%B0%EC%99%84%EC%A0%84%EB%B6%84%EC%84%9D))
    - 1.1 : isUniqChars
    - 1.2 : reverseString
    - 1.3 : anagram
    - 1.4 : encode_space
